...
__Raw Gaia DR3 Data → Data Cleaning → Feature Engineering → Structured Feature Matrix → XGBoost Training → Model Evaluation → Classification of Unseen Data → Candidate Ranking and Storage__
    ...
    In comparison, the initial model developed before the implementation of advanced feature engineering achieved an accuracy of 0.20 and precision of 0.10. The baseline implementation did not include recall, F1 score, or runtime measurements, which limited the ability to comprehensively evaluate its predictive performance.
